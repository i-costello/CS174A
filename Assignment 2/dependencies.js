...
let all_dependencies =  "Triangle Square Tetrahedron Windmill Subdivision_Sphere Cup Disk FullCup Cube Table Text_Line Phong_Model Funny_Shader Movement_Controls Global_Info_Table".split(' ');
...
{ constructor()
    { super();
      ...
      for( var i = 0; i < num_strips; i++ ) {
        var spinA = Mat4.rotation( (i-1) * 2*Math.PI/num_strips, Vec.of( 0, 1, 0 ) );
        var spinB = Mat4.rotation( i * 2*Math.PI/num_strips, Vec.of( 0, 1, 0 ) );
        var newPointA = spinA.times( (Vec.of(radA, 0, 0)).to4(1) ).to3();   // Apply that XZ rotation matrix to point (1,0,0) of the base triangle.
        var newPointB = spinB.times( (Vec.of(radA, 0, 0)).to4(1) ).to3();
        var newPointC = spinA.times( (Vec.of(radB, 1.5, 0)).to4(1) ).to3();
        var newPointD = spinB.times( (Vec.of(radB, 1.5, 0)).to4(1) ).to3();
              this.positions.push( newPointA,                         // Store this XZ position.                  This is point 1.
                                  newPointB,
                                  newPointC,
                                  newPointD );     // Store it again but with higher y coord:  This is point 2.
                                  //Vec.of( 0, 0, 0 )            );  // All triangles touch this location.       This is point 3.

        var newNormalA = spinA.times( Vec.of( 1, -0.25, 0 ).to4(0) ).to3();  
        var newNormalB = spinB.times( Vec.of( 1, -0.25, 0 ).to4(0) ).to3();
        this.normals       .push( newNormalA, newNormalB, newNormalA, newNormalB  );
        //this.texture_coords.push( ...Vec.cast( [ 0, 0 ], [ 0, 1 ], [ 1, 0 ], [ 1, 1 ] ) );
        this.indices       .push( 4*i, 4*i + 1, 4*i + 2             ); // Procedurally connect the three new vertices into triangles.
...
{ constructor()
    { super();
      ...
      for( var i = 0; i < num_strips; i++ ) {
            var spinA = Mat4.rotation( (i-1) * 2*Math.PI/num_strips, Vec.of( 0, 1, 0 ) );
            var spinB = Mat4.rotation( i * 2*Math.PI/num_strips, Vec.of( 0, 1, 0 ) );        // Rotate around a few degrees in XZ plane to place each new point.
            var newPointA = spinA.times( Vec.of( radA, 0, 0 ).to4(1) ).to3();   // Apply that XZ rotation matrix to point (1,0,0) of the base triangle.
            var newPointB = spinB.times( Vec.of( radA, 0, 0 ).to4(1) ).to3();
            this.positions.push( newPointA,                         // Store this XZ position.                  This is point 1.
                                 newPointB,     // Store it again but with higher y coord:  This is point 2.
                                 Vec.of( 0, 0, 0 )            );  // All triangles touch this location.       This is point 3.

            // Rotate our base triangle's normal (0,0,1) to get the new one.  Careful!  Normal vectors are not points; their perpendicularity constraint
            // gives them a mathematical quirk that when applying matrices you have to apply the transposed inverse of that matrix instead.  But right 
            // now we've got a pure rotation matrix, where the inverse and transpose operations cancel out.
            var newNormal = spinA.times( Vec.of( 0, 0, 1 ).to4(0) ).to3();  
            this.normals       .push( newNormal, newNormal, newNormal             );
            this.texture_coords.push( ...Vec.cast( [ 0, 0 ], [ 0, 1 ], [ 1, 0 ] ) );
            this.indices       .push( 3*i, 3*i + 1, 3*i + 2                       ); // Procedurally connect the three new vertices into triangles.
        }
...
class FullCup extends Shape
{ constructor()
  { super();

    var cup_transform = Mat4.set_identity;
    Cup.prototype.insert_transformed_copy_into( this, [], cup_transform );
    Disk.prototype.insert_transformed_copy_into( this, [], cup_transform );
...
class TableLeg extends Shape
{ constructor(num_strips, length)
    { super();
      //var num_strips = 7;
      var radA = 0.5;
      var radB = 0.5;
      for( var i = 0; i < num_strips; i++ ) {
        ...
        var newPointA = spinA.times( (Vec.of(radA, 0, 0)).to4(1) ).to3();   // Apply that XZ rotation matrix to point (1,0,0) of the base triangle.
        var newPointB = spinB.times( (Vec.of(radA, 0, 0)).to4(1) ).to3();
        var newPointC = spinA.times( (Vec.of(radB, length, 0)).to4(1) ).to3();
        var newPointD = spinB.times( (Vec.of(radB, length, 0)).to4(1) ).to3();
              this.positions.push( newPointA,                         // Store this XZ position.                  This is point 1.
                                  newPointB,
                                  newPointC,
                                  newPointD );     // Store it again but with higher y coord:  This is point 2.
...
{ constructor()
  { super();
    ...
    var top_transform = Mat4.scale([5, 0.25, 10]);
    Cube.prototype.insert_transformed_copy_into( this, [], top_transform );
    var leg_transform = Mat4.scale([.75,1.5,.75]);
    for( let i of [-1,1] )
      for( let j of [-1,1] ) {
      var temp_transform = Mat4.translation([i*4, -6.25, j*9]).post_multiply(leg_transform);
      TableLeg.prototype.insert_transformed_copy_into( this, [7, 4], temp_transform);
      }
    }
}
...
{ constructor( context, canvas = context.canvas )
    { super( context );                            // Initialize some data members:
      Object.assign( this, { thrust: Vec.of(0,0,0), roll: 0, origin: Vec.of(0,5,0), look_around_locked: true, pos: Vec.of( 0,0,0 ), z_axis: Vec.of( 0,0,0 ), thrown: 0 } );
      this.target = function() { return context.globals.movement_controls_target() }                                      // The camera matrix is not actually stored here inside Movement_Controls; instead, trackZ
      ...
      //this.ball_is_thrown = function() { return thrown };
  ...
  make_control_panel()   // This function of a scene sets up its keyboard shortcuts.
    { const globals = this.globals;
      ...
      //this.key_triggered_button( "Shoot",    "space", function() { this.thrown = 1; } ); this.new_line();
...
{ make_control_panel()
    { const globals = this.globals;
      ...
      this.live_string( () => { return "Animation Time: " + ( globals.graphics_state.animation_time/1000 ).toFixed(3) + "s" } ); this.new_line();
      this.live_string( () => { return "Frame Rate: "      + ( 1000/globals.graphics_state.animation_delta_time).toFixed(0) + " fps"} );
      ...
      this.key_triggered_button( "Party shading",     "ALT+n", function() { globals.graphics_state.color_normals ^= 1;         } ); this.new_line();
      this.key_triggered_button( "Shoot",    "space", function() 
        { /*if (globals.graphics_state.animation_time/1000 - globals.graphics_state.throw_time/1000 > 2) {
            globals.graphics_state.ball_in_air = 0; 
            globals.graphics_state.thrown = 0;
          }*/
          if (globals.graphics_state.ball_count > 0 && !globals.graphics_state.ball_in_air && globals.graphics_state.game_state != 1 && globals.graphics_state.game_state != 3 && globals.graphics_state.game_state > -1) {
          globals.graphics_state.thrown = 1; 
          globals.graphics_state.throw_time = globals.graphics_state.animation_time;
          globals.graphics_state.ball_in_air = 1;
          globals.graphics_state.ball_count--; }
          if (globals.graphics_state.game_state < 0)
            globals.graphics_state.game_state=0;
        }); this.new_line();

      this.key_triggered_button( "Reload Ball", "enter", function() {
          if (globals.graphics_state.game_state != 1 && globals.graphics_state.game_state != 3) {
          globals.graphics_state.thrown = 0;
          globals.graphics_state.ball_in_air = 0; }
          if ( globals.graphics_state.game_state < 0 || globals.graphics_state.game_state == 3 )
            globals.graphics_state.game_state++;
        });
      //this.key_triggered_button( "Reset Game", "r", function() {var reset = new graphics_state(Mat4.translation([ 0, -5,-30 ]).times(Mat4.rotation(.3, Vec.of(1,0,0))), Mat4.perspective( Math.PI/4, context.width/context.height, .1, 1000 )); display(reset);});
    }
}